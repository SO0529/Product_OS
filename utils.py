...
def prepare_data(dataset):
    """
    Args:
      :param dataset: choose train dataset or test dataset
...
def imread(path, is_grayscale=True):
    """
    Args:
      :param path: file path of reading images
...
def input_setup(config):
    ...
    if config.is_train:
        for i in xrange(len(data)):
            ...
            # 学習用の小さい画像[33 x 33]と畳み込み後の正解ラベル[21 x 21]を作成
